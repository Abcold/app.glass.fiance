...
export const useEther = (chainId) => computed(() => {
    if (isMatic(chainId.value)) {
        return new MaticNativeCurrency(chainId.value)
    } else if (isBnb(chainId.value)) {
        return new BnbNativeCurrency(chainId.value)
    } else {
        return new Ether(chainId.value)
    }
})
